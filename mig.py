    ...
    if query:
        if status == 'delete':
            if query['status'] == 'CREATED':
                ...
                print 'INFO: Migration {} deleted.'.format(migration_id)
            else:
...
def aci_precheck(migration_id):
    ...
    Notes:
        ...
        switch_ports:
        Ports in use (configured) for each node. 'local' means directly attached (no FEX).
        {'LEAF_220': {'local': ['35', '36', '37', '38', '40', '41', '43', '11'],
                      'nodes': ['220']},
         'LEAF_221': {'local': ['40', '11', '43'],
                      'nodes': ['221']},
         'LEAF_222': {'101': ['1','2','3'],
                      'local': ['8', '3', '20', '7'],
                      'nodes': ['222']},
    ...
    Returns:
        {'rc': return_code,         0 - success, 1 - error
         'info': msgs,              List of INFO messages
         'warning': warning_msgs,   List of WARNING messages
         'error': error_msgs,       List of ERROR messages
         'config': aci_config_args  ACI configuration arguments
         }
...
def main():
    ...
    parser_import = subparsers.add_parser("import", help="Import migration data file.")
    parser_import.add_argument( "-file", help="Data file containing ACI switch, ACI port, Legacy Switch, Legacy Port columns (*.xlsx).")
    ...
    # precheck
    parser_precheck = subparsers.add_parser("precheck", help="Pre-check DB, metadata and ACI configuration.")
    parser_precheck.add_argument("id", help="Migration id")

    # print config
    parser_print_config = subparsers.add_parser("print_config", help="Print ACI configuration arguments.")
    ...
    parser_config.add_argument("-tasks", choices=["fabric","tenant", "bindings", "bd_subnets", "arp_flood_off", "arp_flood_on"],
    ...
    if args.subcommand == 'import':
        ...
        if args.list:
            ...
            set_migration_status(args.delete, status='delete')
