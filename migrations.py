...
def load_patching_file_csv(file_name, vrf, description):
    ...
    try:
        patching_file = open(file_name, 'r')

        for line in patching_file.readlines():
            if line and not line.startswith('#'):
                patching_line = line.strip('\n').split(',')
                device_name = patching_line[1]
                interface_temp = patching_line[2]
                result = interface_name_normalizer(interface_temp)
                if result[0] == 0:
                    interface = result[1]
                else:
                    return [1, result[1]]
                aci_node = patching_line[6]
                aci_interface  = patching_line[7]
...
def patching_validate(patching):
    ...
    for item in patching:
        if {'device_name': item['device_name']} not in switches:
            switches.append({'device_name': item['device_name']})
        switch_interface_list.append('{0}@{1}'.format(item['device_name'], item['interface']))
        node_interface_list.append('{0}@{1}'.format(item['aci_node'], item['aci_interface']))
    ...
    for item in patching:
        ...
        query = interfaces.find_one({'device_name': item['device_name'], 'interface': item['interface']})
        if not query:
            msgs.append('ERROR: Interface {0} of Device {1} not in Discovery DB'.format(item['interface'],
                                                                                       item['device_name']))
        else:
            # Only HOST interfaces allowed.
            if query['interface_type'] != 'HOST':
                msgs.append('ERROR: Interface {0} of Device {1} not HOST port'.format(item['interface'],
                                                                                     item['device_name']))
            else:
                ...
                result = speed_normaliser(query['speed'])
                if result[0] == 0:
                    std_speed = result[1]
                if not std_speed:
                    ...
                    msg = 'ERROR: 10M interfaces not supported on ACI.'
                    msgs.append(msg)
                elif std_speed == '100M' and query['duplex'] == 'half':
                    msg = 'ERROR: 100M half not supported on ACI'
                    msgs.append(msg)
                ...
                if 'vpc' in query:
                    ...
                    # {vpc: number} eg {'126': 2}
                    if item['vpc']:
                        if item['vpc'] in vpc_intf_counter:
                            vpc_intf_counter[item['vpc']] += 1
                        else:
                            vpc_intf_counter[item['vpc']] = 1
                ...
                if 'lag' in query:
                    ...
                    # {lag: number} eg {'126': 2}
                    if item['lag']:
                        if item['lag'] in lag_intf_counter:
                            lag_intf_counter[item['lag']] += 1
                        else:
                            lag_intf_counter[item['lag']] = 1
    ...
    for item in patching:
        ...
        query = migrations_data.find_one({'device_name': item['device_name'], 'interface': item['interface']})
        if query:
            msgs.append('ERROR: Interface {0} of Device {1} already in Migration {2}.'.format(item['interface'],
                                                                                             item['device_name'],                                                                                          query['migration_id']))
    ...
    if query:
        ...
        if len(mig_data) > 0:
            ...
            for item in mig_data:
                ...
                if intf_query:
                    item['vpc'] = intf_query['vpc']
                    item['lag'] = intf_query['lag']
            ...
            if tenant_exists[0] == 0:
                if not tenant_exists[1]:
                    ...
                    msgs.append('tenant in migration:')
                    msgs.append(tenant)
            ...
            if ap_exists[0] == 0:
                if not ap_exists[1]:
                    ...
                    msgs.append('application profile in migration:')
                    msgs.append(ap)

            # Check each interface
            msgs.append('interfaces in migration:')
            ...
            for item in mig_data:
                ...
                aci_node = item['aci_node']
                aci_interface = item['aci_interface']
                aci_port = item['aci_interface'].split('/')[1]
                ...
                if intf_query:
                    ...
                    if vlans_parsing_result:
                        if vlans_parsing_result[0] == 0:
                            vlans.extend(vlans_parsing_result[1])
                            vlans_per_interface = vlans_parsing_result[1]
                        else:
                            error_msgs.include(vlans_parsing_result[1])
                    # Calculate domain suffixes used to make EPG and BD names
                    for vlan in vlans_per_interface:
                        ...
                        if vlan_query:
                            if vlan_query['subnet_ip']:
                                ...
                                suffix = ip_network
                                domain_suffixes_per_interface.append(suffix)
                            else:
                            ...
                            bindings[vlan] = {'bd': bridge_domain_name, 'epg': epg_name}
                    ...
                    if aci_port in used_ports:
                        msg = 'WARNING: ACI port {} on node {} already assigned.'.format(aci_port, aci_node)
                        msgs.append(msg)
                    ...
                    # Build list of all domain suffixes use in this migration.
                    # Can use this list to calculate EPG and BD names.
                    domain_suffixes.extend(domain_suffixes_per_interface)

            msgs.append('vrf in migration:')
            msgs.append(vrf)


            # Calculate epg and bridge domain names.
            domain_suffixes_set = set(domain_suffixes)
            domain_suffixes_unique = list(domain_suffixes_set)
            bridge_domains = []
            epgs = []
            for suffix in domain_suffixes_unique:
                bd = 'BD_{0}'.format(suffix)
                epg = 'EPG_{0}'.format(suffix)
                bridge_domains.append(bd)
                epgs.append(epg)
                aci_config_args['epg_bridge'].append({'epg': epg, 'bd': bd})
            msgs.append('bridge domains in migration:')
            msgs.append(','.join(bridge_domains))
            msgs.append('epgs in migration:')
            msgs.append(','.join(epgs))
...
def aci_configure(migration_id, aci_config_args, tasks):
    ...
    def object_checker(object_type, **kwargs):
        tenant_name = app_name = epg_name = bd_name = vrf_name = l3_out = phys_dom = path = pod = nodes = port = policy_group = vlan = trunk = ''
        aep = ll_policy = cdp_policy = mcp_policy = lldp_policy = stp_policy = lacp_policy = interface_profile = interface_selector = ''
        if 'aep' in kwargs:
            aep = kwargs['aep']
    ...
    if 'tenant' in tasks:
        ...
        for epg_bridge in aci_config_args['epg_bridge']:
            bd = epg_bridge['bd']
    ...
    if 'bindings' in tasks:
        ...
        for interface in aci_config_args['interface']:
            policy_group = interface['policy_group']
            for vlan, epg_bridge in interface['bindings'].items():
                epg = epg_bridge['epg']
                if interface['channel'] == 'single':
...
def aci_prepare_fabric():
    ...
    print 'set_leaf_access_port_policy_grp:', apic.aci_set_leaf_access_port_policy_grp('IPG_ACC_1G_ON',
                                                                                       'AEP_COMPUTE',
                                                                                       '1gig_auto',
                                                                                       'cdp_enabled',
                                                                                       'mcp_enabled',
